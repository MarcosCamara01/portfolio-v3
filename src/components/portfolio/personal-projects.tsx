    ...
    <div className="flex flex-col gap-5">
      {projectsData.personalProjects.map((project: Project) => (
        <Link
          ...
          key={project.id}
          target="_blank"
          className="transition-[background-color] hover:bg-foreground border-2 border-foreground p-4 flex-col flex gap-5 rounded"
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {project.stack.map((tech) => (
                <span
                  key={tech}
                  className="text-xs font-medium bg-color-secondary py-1 px-3 rounded-full"
                >
                  {tech}
                </span>
              ))}
